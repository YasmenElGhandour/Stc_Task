...
@Singleton
class PostsAdapter @Inject constructor() : PagingDataAdapter<PostsResult, PostsAdapter.ViewHolder>(differCallback) {
    ...
    inner class ViewHolder : RecyclerView.ViewHolder(this.binding.root) {
        fun bind(item: PostsResult) {
            binding?.apply {
                Glide.with(context).load(Constants.IMAGES_BASE_URL + item.image).into(binding.itemImage)
